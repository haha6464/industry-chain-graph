...
- 同一父节点下兄弟节点粒度必须一致；整体目标是 L0-L5 的 5-6 层图谱。一级分支通常至少展开到 L3，核心供给/生产转换/关键材料/核心零部件/专用设备/重要产品或服务应到 L4，少数核心长链可到 L5；渠道/物流/检测/运维/咨询等支撑环节通常止于 L3-L4。若所有分支都最多到 L3，应判为整体深度过浅。
""".strip()
...
判定规则：只要出现明显不适合作为投研产业链节点的名称、同级粒度严重不一致、同一节点多父节点、核心分支明显过浅、全图或核心分支全部止于 L3、支撑/渠道分支无依据地过深，应返回 needs_revision。
