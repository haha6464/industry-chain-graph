...
3. 同一公司可挂多个相互独立的业务节点；若同时匹配父节点和子节点，只返回层级更深且业务仍准确匹配的节点。所有层级一视同仁，L2、L3、L4 均可作为直接挂载目标；不得为了追求更深层级而牺牲主营业务匹配准确性。
4. 不得返回 level=0 根节点；没有具体匹配时 matched_nodes 返回空数组。
5. 只有在该公司主营业务确实跨越或不属于任何现有更深节点时才允许直挂 L1；此时 reason 必须明确说明为何现有更深节点均不适配。reason 使用不超过 40 个汉字说明主营业务与节点的直接关系；confidence 为 0.5 到 1.0。
