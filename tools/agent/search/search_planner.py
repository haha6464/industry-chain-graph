...
LEVEL_EXTRACTION_POLICY = {
    ...
    "target_depth": "目标整体形成 L0-L5 的 5-6 层图谱；多数一级分支至少展开到 L3，核心供给、生产/转换、关键材料、核心零部件、专用设备、重要产品或服务等证据充分时应到 L4，少数核心长链可到 L5；渠道、物流、检测认证、运维、咨询等支撑环节通常止于 L3-L4，不硬凑但也不能全图止于 L3。",
    "not_level_names": "不要把上游/中游/下游作为 level 名称；它们只用于描述节点间流向关系或 chain_position 位置标签。",
    "target_node_count": "目标节点数量 60-100 个，硬上限 150 个。",
    "breadth_requirement": "不要只沿少数分支深挖；level=1 应覆盖主要投研分析环节，重要一级环节应展开 3-8 个二级/三级兄弟分支，并在其中选择核心子链继续下钻到 L4/L5。优先补齐横向缺失的上游供给、关键材料/零部件、工艺/技术路线、设备/基础设施、产品或服务形态、下游应用/需求和必要支撑服务；避免公司、品牌、新闻政策、市场规模、平台能力、泛咨询、SaaS/解决方案等非产业链节点。",
}
